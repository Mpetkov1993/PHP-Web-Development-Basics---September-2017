...
class Carshop {
    ...
    protected function getSales() {
        $stmt = $this->db->prepare("
        SELECT `car_make`,`car_model`,`sale_date`,`amount`
        FROM `cars`.`used_cars`, `cars`.`sales`
        WHERE `used_cars`.`car_id` = `sales`.`car_id`");
        $stmt->execute();
        $out = "";
        $total = 0;
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $total += explode(' ', trim($row['amount']))[1];
        }
        $out .= str_repeat('-', 30).PHP_EOL;
        $out .= 'Total amount: '.$total.PHP_EOL;
        return $out;
    }
